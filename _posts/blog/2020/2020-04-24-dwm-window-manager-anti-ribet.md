...
> INFO
> 
> Saya hanya mengambil salah satu paragraf dari filosofi DWM, lebih lengkapnya teman-teman dapat menbacanya sendiri [**di sini**](https://suckless.org/philosophy/).
...
1. Mengelola banyak *patch* sangat melelahkan. Mungkin dikarenakan saya belum memahami, bagaimana *best practice* dalam mengelola dan mengaplikasikan *patch.
1. Saya tidak ingin menambahkan *patch* yang saya tidak benar-benar perlukan.
...
1. **suckdiff** : untuk membuat backup branch dalam bentuk *patch* yang tersimpan di `~/.config/suckless/`
1. **suckmerge2** : untuk me-*merge*-kan branch-branch terpilih ke master branch, sekaligus mengcompilenya
...
Created by: BanditHijo, versi terbaru [di sini](https://github.com/bandithijo/sucklessthing/blob/master/suckmerge2)
...
1. Jalankan [`suckmerge2`](https://github.com/bandithijo/sucklessthing/)*
1. Apabila berhasil, restart dwm.
1. Saat ini master branch dalam keadaan "kotor", jalankan [`suckclean`](https://github.com/bandithijo/sucklessthing/)* untuk mereset dan membersihkannya.
1. Untuk mengedit *patch* branch dengan cara `git checkout <nama_branch>`, wajib menjalankan `suckclean` terlebih dahulu.
...
Sejauh yang saya ingat, ada satu *patch* yang saya gunakan namun bukan dari halaman website suckless. Yaitu [**GasparVardanyan/dwm-scratchpad**](https://github.com/GasparVardanyan/dwm-scratchpad).
...
Saya mendapatkan patch ini dari post Reddit yang berjudul ["dwm center floating window with multiple monitors"](https://www.reddit.com/r/suckless/comments/cphe3h/dwm_center_floating_window_with_multiple_monitors/).
...
```bash
!filename: $HOME/.local/bin/dwmstatus
#!/usr/bin/env bash
...
Saya menggunakan [**aur/ifstat**](https://aur.archlinux.org/packages/ifstat/).

**cpu-temp.sh** - Created by: BanditHijo

```bash
!filename: cpu-temp.sh
#!/bin/sh
...
```bash
!filename: $HOME/.local/share/dwm/autostart.sh
#!/bin/sh
...
Bisa teman-teman lihat [di sini](https://github.com/bandithijo/dwm).
...
Versi terbaru dari **suckchkout** dapat teman-teman temukan [di sini](https://github.com/bandithijo/sucklessthing/blob/master/suckchkout)
...
1. [**dwm**](https://github.com/bandithijo/dwm) \
...
2. [**sucklessthing**](https://github.com/bandithijo/sucklessthing/) \
...
3. [**dmenu**](https://github.com/bandithijo/dmenu) \
    Repo ini berisi master dmenu dengan branch config yang sudah saya modifikasi.

4. [**slstatus**](https://github.com/bandithijo/slstatus) \
